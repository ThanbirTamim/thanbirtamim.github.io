/* ============================================================
   CHESS — networking layer (WebRTC via PeerJS)
   Host-authoritative star topology (2 players). Browsers connect
   directly via P2P DataChannels; PeerJS's free broker is used ONLY
   for signalling. No backend / database / accounts.
   ------------------------------------------------------------
...
(function (root) {
  "use strict";
  const CFG = root.CHESS_CONFIG;

  class ChessNet {
    constructor() {
      this.peer = null; this.isHost = false; this.roomCode = null; this.passcode = null;
      this.myId = null; this.hostId = null; this.conns = {};
      this.hostConn = null; this.handlers = {}; this.maxPlayers = 2; this.closed = false;
      this.roster = []; this.started = false;
    }
    ...
    createRoom(opts) {
      ...
      this.isHost = true; this.maxPlayers = 2; this.passcode = opts.passcode || null;
      this.hostColor = opts.color || "w"; this.timeControl = opts.timeControl || "off";
    ...
    _hostOnConn(conn) {
      ...
      conn.on("close", () => { delete this.conns[conn.peer]; const e = this.roster.find((r) => r.peerId === conn.peer); if (e) e.connected = false; this._broadcastRoster(); this.emit("peer-left", { peerId: conn.peer }); });
      conn.on("error", () => {});
    }
    _hostHandle(conn, msg) {
      if (msg.t === "join") {
        ...
        if (existing) { existing.peerId = conn.peer; existing.connected = true; conn.send({ t: "welcome", you: existing.pid, roster: this.roster, code: this.roomCode, started: this.started, max: this.maxPlayers, hostColor: this.hostColor, timeControl: this.timeControl }); this._broadcastRoster(); this.emit("peer-rejoined", { pid: existing.pid }); if (this.started && this.lastState) conn.send({ t: "state", state: this.lastState }); return; }
        ...
        const pid = "P2-" + Math.random().toString(36).slice(2, 6);
        const entry = { pid, peerId: conn.peer, name: (msg.name || "Player").slice(0, 14), connected: true };
        this.roster.push(entry);
        conn.send({ t: "welcome", you: pid, roster: this.roster, code: this.roomCode, started: false, max: this.maxPlayers, hostColor: this.hostColor, timeControl: this.timeControl });
    ...
    broadcastEmote(pid, content) { this._send({ t: "emote", pid, content }); }
    ...
    _connectHost() {
      ...
      const timeout = setTimeout(() => { if (!this._welcomed) this.emit("error", { code: "timeout", msg: "Couldn't connect. Tip: make sure the host is on the lobby screen with the code. Different networks (e.g. mobile data) may need a TURN server — see README." }); }, CFG.NET.joinTimeoutMs || 20000);
    ...
    _clientHandle(msg) {
      if (msg.t === "welcome") { this._welcomed = true; this.myPid = msg.you; this.roster = msg.roster; if (msg.max) this.maxPlayers = msg.max; this.hostColor = msg.hostColor; this.timeControl = msg.timeControl; this.emit("welcome", msg); }
      else if (msg.t === "join-reject") { const m = { full: "This room already has two players.", started: "The game has already started.", passcode: "Wrong passcode." }[msg.reason] || "Could not join."; this.emit("error", { code: "reject-" + msg.reason, msg: m }); }
